...
SELECT   totalBBCount.cnt# total, bbCountWithProfile.cnt# withProfile#,totalBBCount.cnt#-bbCountWithProfile.cnt# withoutProfile#  from               
(SELECT 
  count(distinct concat(max(alx.id),alx.subscriberid)) cnt#
 ...
 -- and alx.application in 'NPCA' 
  ...
  -- and alx.operation not like 'ORDER:%'
    ...
    order by rownum ) bbCountWithProfile,
